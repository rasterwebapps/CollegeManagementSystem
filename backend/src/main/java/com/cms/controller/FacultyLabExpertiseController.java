...
public class FacultyLabExpertiseController {
    ...
    public FacultyLabExpertiseController(FacultyLabExpertiseService facultyLabExpertiseService) {
        this.facultyLabExpertiseService = facultyLabExpertiseService;
    }

    @GetMapping
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_FACULTY')")
    public ResponseEntity<List<FacultyLabExpertiseResponse>> findAll() {
        return ResponseEntity.ok(facultyLabExpertiseService.findAll());
    }
    ...
    public ResponseEntity<FacultyLabExpertiseResponse> create(
        ...
        return ResponseEntity.status(HttpStatus.CREATED).body(facultyLabExpertiseService.create(request));
    }

    @PutMapping("/{id}")
    @PreAuthorize("hasRole('ROLE_ADMIN')")
    public ResponseEntity<FacultyLabExpertiseResponse> update(
            @PathVariable Long id, @Valid @RequestBody FacultyLabExpertiseRequest request) {

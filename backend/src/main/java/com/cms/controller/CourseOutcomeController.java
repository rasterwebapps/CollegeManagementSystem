...
public class CourseOutcomeController {
    ...
    public ResponseEntity<List<CourseOutcomeResponse>> findAll() {
        return ResponseEntity.ok(courseOutcomeService.findAll());
    }
    ...
    @PostMapping
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_FACULTY')")
    public ResponseEntity<CourseOutcomeResponse> create(@Valid @RequestBody CourseOutcomeRequest request) {
        return ResponseEntity.status(HttpStatus.CREATED).body(courseOutcomeService.create(request));
    }

    @PutMapping("/{id}")
    @PreAuthorize("hasAnyRole('ROLE_ADMIN', 'ROLE_FACULTY')")
    public ResponseEntity<CourseOutcomeResponse> update(@PathVariable Long id,
